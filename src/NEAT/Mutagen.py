        ...
        if random.random() < self.mutation_chance:
            ...
            if self.value_type == ValueType.WHOLE_NUMBERS:
                if random.random() < 0.2:
                    ...
                    new_current_value = self.current_value + int(
                        (math.pow(random.random(), 3) - 0.5) * (1 + self.end_range - self.start_range))

                if new_current_value == self.current_value:
                    new_current_value = (self.current_value + (
                        1 if random.random() < 0.5 else -1) - self.start_range) % (
                                                self.end_range - self.start_range) + self.start_range
            ...
            if self.value_type == ValueType.CONTINUOUS:
                if random.random() < 0.1:
                    ...
                    new_current_value = self.current_value + (math.pow(random.random(), 3) - 0.5) * (
