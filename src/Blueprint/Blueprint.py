...
class BlueprintNode(Node):
    ...
    def parseto_module(self, generation, moduleConstruct=None, speciesindexes=None):
        ...
        if(self.module_root is None and self.module_leaf is None):
            ...
            inputModuleNode, index = generation.speciesCollection[self.value].sample_module()  # to be added as child to existing module construct
        ...
        if(first_traversal):
            for childBlueprintNode in self.children:
                childBlueprintNode.parseto_module(generation, outputModuleNode,speciesindexes)  # passes species index down to collect all species indexes used to construct this blueprint in one list
