...
def refine_feature_multiplication_guess(feature_mulitplication_guess, blueprint: BlueprintGenome, sample_map: Dict[int, int],
    ...
    if remaining_tries == 0:
        return best_guess
    model: Network = Network(blueprint, input_size, feature_multiplier=feature_mulitplication_guess, sample_map=sample_map).to(config.get_device())
    model_size = sum(p.numel() for p in model.parameters() if p.requires_grad)
    ...
    return refine_feature_multiplication_guess(next_guess, blueprint, sample_map, input_size,
                                               remaining_tries=remaining_tries-1,
                                               best_guess=best_guess, best_target_ratio=best_target_ratio)


def get_model_of_target_size(blueprint: BlueprintGenome, sample_map: Dict[int, int], original_model_size, input_size: List[int]) -> Network:
    feature_mulitplication_first_guess = pow(config.target_network_size/ original_model_size, 0.5)
    feature_mult_best_approximation = refine_feature_multiplication_guess(feature_mulitplication_first_guess, blueprint,sample_map ,input_size)

    model: Network = Network(blueprint, input_size, feature_multiplier=feature_mult_best_approximation, sample_map= sample_map).to(config.get_device())
    model_size = sum(p.numel() for p in model.parameters() if p.requires_grad)

    print("targeting size using feature mult:",feature_mult_best_approximation,"original size:", original_model_size,
          "normalised size:",model_size,"target:",config.target_network_size, "change ratio:",
          (model_size/original_model_size), "target ratio:",(model_size/config.target_network_size))
    return model
